...
public class DetectorScenes {
    ...
    public static void smartObserver(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showText(60).text("Smart Observers can be used to detect a variety of events")
            .pointAt(util.vector().blockSurface(observerPos, Direction.WEST)).placeNearTarget();
        scene.idle(50);
        ...
        scene.overlay().showText(60).text("It can detect items or fluids inside of generic containers").attachKeyFrame()
            .pointAt(util.vector().blockSurface(observerPos, Direction.WEST)).placeNearTarget();
        scene.idle(65);
        ...
        scene.overlay().showText(50).text("It also activates when the block itself matches the filter").attachKeyFrame()
            .pointAt(util.vector().blockSurface(observerPos.north(), Direction.WEST)).placeNearTarget();
    ...
    public static void thresholdSwitch(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection cogs = util.select().fromTo(4, 0, 6, 5, 0, 6).add(util.select().position(5, 0, 5));
        ...
        Selection basePlate = util.select().fromTo(0, 0, 1, 2, 0, 5).add(util.select().fromTo(4, 0, 5, 4, 0, 1));
        ...
        scene.overlay().showText(60).text("Threshold Switches monitor the fill level of containers")
            .pointAt(util.vector().blockSurface(switchPos, Direction.NORTH)).placeNearTarget();
        scene.idle(60);
        ...
        scene.overlay().showText(50).text("...the switch will change its redstone output")
            .pointAt(util.vector().blockSurface(switchPos.north(), Direction.DOWN)).placeNearTarget();
        scene.idle(50);
        ...
        scene.overlay().showText(70).text("A filter can help to only count specific contents toward the total")
            .pointAt(util.vector().blockSurface(switchPos, Direction.UP)).attachKeyFrame().placeNearTarget();
        ...
        scene.overlay().showText(70).text("Fluid buffers can be monitored in a similar fashion")
            .pointAt(util.vector().blockSurface(switchPos, Direction.NORTH)).attachKeyFrame().placeNearTarget();
        scene.idle(80);
