...
import com.simibubi.create.client.ponder.api.element.*;
...
public class FanScenes {
    ...
    public static void direction(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        scene.world()
            .showSection(util.select().fromTo(3, 1, 0, 3, 1, 5).add(util.select().position(3, 2, 4)), Direction.DOWN);
        ...
        ElementLink<ParrotElement> flappyBirb = scene.special()
            .createBirb(util.vector().topOf(1, 0, 3), ParrotPose.FlappyPose::new);
        scene.idle(2);
        scene.special().rotateParrot(flappyBirb, 0, 235, 0, 30);
        scene.special().moveParrot(flappyBirb, util.vector().of(0, 0, -2.5), 30);
        scene.idle(20);

        scene.overlay().showText(80).text("Encased Fans use Rotational Force to create an Air Current").attachKeyFrame()
            .placeNearTarget().pointAt(util.vector().topOf(fanPos));
        scene.idle(90);
        ...
        scene.overlay().showText(60).text("Strength and Direction of Flow depends on the Rotational Input")
            .placeNearTarget().pointAt(util.vector().topOf(fanPos));
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        scene.world()
            .showSection(util.select().fromTo(6, 1, 2, 5, 1, 2).add(util.select().position(1, 1, 2)), Direction.DOWN);
        scene.idle(25);
        ...
        ElementLink<WorldSectionElement> blockInFront = scene.world()
        ...
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 80)
            .colored(PonderPalette.RED).text("When passing through lava, the Air Flow becomes Heated");
        scene.idle(80);
        ...
        scene.effects().emitParticles(
            itemVec.add(0, 0.2f, 0),
            scene.effects().simpleParticleEmitter(ParticleTypes.LARGE_SMOKE, Vec3.ZERO),
            1,
            60
        );
        ...
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60)
            .colored(PonderPalette.BLACK).attachKeyFrame()
        ...
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60)
            .colored(PonderPalette.MEDIUM).attachKeyFrame()
        ...
        scene.effects().emitParticles(
            itemVec.add(0, 0.2f, 0),
            scene.effects().simpleParticleEmitter(ParticleTypes.SPIT, Vec3.ZERO),
            1,
            60
        );
        ...
        ElementLink<WorldSectionElement> depot = scene.world()
            .showIndependentSection(util.select().position(depos), Direction.DOWN);
        ...
        scene.idle(15);
        ElementLink<WorldSectionElement> largeCog = scene.world()
            .showIndependentSection(util.select().position(1, 2, 4), Direction.UP);
        ElementLink<WorldSectionElement> belt = scene.world()
            .showIndependentSection(util.select().fromTo(3, 3, 1, 1, 3, 3), Direction.DOWN);
        scene.world().moveSection(largeCog, util.vector().of(-1, -2, 0), 0);
        scene.world().moveSection(belt, util.vector().of(-1, -2, 0), 0);
        ElementLink<BeltItemElement> transported = scene.world()
