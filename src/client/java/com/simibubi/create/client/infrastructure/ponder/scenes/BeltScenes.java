...
public class BeltScenes {
    ...
    public static void beltConnector(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(util.vector().topOf(shaftLocation), Pointing.DOWN, 50).rightClick()
            .withItem(AllItems.SHAFT.getDefaultInstance());
        scene.idle(7);
    ...
    public static void directions(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.overlay().showText(40).colored(PonderPalette.GREEN).placeNearTarget().pointAt(to).attachKeyFrame()
        ...
        scene.idle(10);
        scene.overlay().showText(60).colored(PonderPalette.GREEN).placeNearTarget().pointAt(to).attachKeyFrame()
    ...
    public static void transport(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showText(60).placeNearTarget().pointAt(util.vector().topOf(beltEnd))
        ...
        scene.idle(10);
        scene.overlay().showText(60).placeNearTarget().pointAt(topOf.subtract(0, 0.1, 0)).attachKeyFrame()
    ...
    public static void beltsCanBeEncased(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntityNBT(
            util.select().position(beltPos2), BeltBlockEntity.class, nbt -> {
                nbt.store("Casing", BeltBlockEntity.CasingType.CODEC, BeltBlockEntity.CasingType.ANDESITE);
            }
        );
        scene.idle(20);
        ...
        for (BlockPos pos : andesiteBelts) {
            ...
            scene.world().modifyBlockEntityNBT(
                util.select().position(pos), BeltBlockEntity.class, nbt -> {
                    nbt.store("Casing", BeltBlockEntity.CasingType.CODEC, BeltBlockEntity.CasingType.ANDESITE);
                }
            );
        }
        ...
        scene.overlay().showText(80).text("A wrench can be used to remove the casing").placeNearTarget()
    ...
    public static void depot(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        ElementLink<WorldSectionElement> spout = scene.world()
            .showIndependentSection(util.select().position(depotPos.above(2).west()), Direction.SOUTH);
