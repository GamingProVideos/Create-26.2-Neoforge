...
public class PostboxScenes {
    ...
    public static void postbox(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        BlockPos controls = util.grid().at(2, 3, 5);
        ...
        ElementLink<WorldSectionElement> postboxE = scene.world().showIndependentSection(boxS, Direction.DOWN);
        ...
        scene.idle(10);
        scene.world().showSectionAndMerge(girder, Direction.NORTH, base);
        scene.idle(5);
        scene.world().showSectionAndMerge(largeCog, Direction.UP, base);
        scene.world().showSectionAndMerge(cog, Direction.DOWN, base);
        scene.world().showSectionAndMerge(belt, Direction.WEST, base);
        scene.idle(5);

        scene.world().showSectionAndMerge(util.select().position(funnel), Direction.DOWN, base);
        scene.idle(10);

        ItemStack boxItem = PackageStyles.getDefaultBox().copy();
        ...
        scene.idle(1);
        scene.special().moveParrot(birbL, util.vector().of(-5, 0, 0), 25);
        scene.world().moveSection(train1L, util.vector().of(-5, 0, 0), 25);
        scene.world().animateBogey(util.grid().at(2, 2, 5), 6, 25);
        scene.idle(14);
        ElementLink<WorldSectionElement> train2L = scene.world().showIndependentSection(train2, null);
        scene.world().moveSection(train2L, util.vector().of(3, 0, 0), 0);
        scene.idle(1);
        scene.world().moveSection(train2L, util.vector().of(-2, 0, 0), 10);
        scene.world().animateBogey(util.grid().at(6, 2, 5), 2, 10);

        scene.idle(10);
        scene.special().moveParrot(birbL, util.vector().of(-1, 0, 0), 10);
        scene.world().moveSection(train1L, util.vector().of(-1, 0, 0), 10);
        scene.world().animateBogey(util.grid().at(2, 2, 5), 1, 10);
        scene.world().moveSection(train2L, util.vector().of(-1, 0, 0), 10);
        scene.world().animateBogey(util.grid().at(6, 2, 5), 1, 10);
        scene.idle(10);
        ...
        scene.world().createItemOnBeltLike(util.grid().at(5, 2, 4), Direction.DOWN, boxItem);

        scene.idle(15);
        animatePostbox(scene, box, false);
        scene.overlay().showText(80).text("..trains stopping at the station will collect it as cargo").attachKeyFrame()
        ...
        scene.world().createItemOnBelt(util.grid().at(2, 1, 2), Direction.WEST, boxItem2);
        animatePostbox(scene, box, false);
        scene.idle(25);
        ...
        ElementLink<WorldSectionElement> tracksL2 = scene.world().showIndependentSection(tracks, Direction.EAST);
        scene.world().moveSection(tracksL2, util.vector().of(-31, 0, 0), 0);

        scene.idle(15);
        scene.world().animateTrainStation(station, false);
        scene.world().moveSection(tracksL, util.vector().of(12, 0, 0), 120);
        ...
        scene.world().moveSection(outpostL, util.vector().of(-4, -1, -1), 0);
        scene.idle(1);
    ...
    public static void animatePostbox(CreateSceneBuilder scene, BlockPos box, boolean raise) {
        scene.world().modifyBlockEntity(box, PostboxBlockEntity.class, be -> be.forceFlag = raise);
    }
