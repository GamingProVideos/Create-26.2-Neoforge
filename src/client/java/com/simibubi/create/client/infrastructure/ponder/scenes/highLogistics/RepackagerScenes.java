...
public class RepackagerScenes {
    ...
    public static void repackager(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        Selection belt3 = util.select().fromTo(5, 1, 1, 5, 1, 6).add(util.select().fromTo(6, 1, 5, 6, 1, 6));
        Selection crafterCogs = util.select().fromTo(1, 1, 6, 0, 2, 6).add(util.select().position(0, 2, 5))
            .add(util.select().fromTo(1, 1, 7, 2, 0, 7));
        Selection crafter = util.select().fromTo(1, 2, 5, 3, 4, 5);
        Selection crafterScaff = util.select().fromTo(3, 1, 5, 1, 1, 5);
        Selection packager = util.select().fromTo(4, 1, 5, 4, 2, 5);
        BlockPos pack = util.grid().at(4, 2, 5);
        Selection packFunnel = util.select().position(5, 2, 5);
        Selection largeCog1 = util.select().position(0, 0, 2);
        Selection largeCog2 = util.select().position(8, 0, 2);
        BlockPos funnel1 = util.grid().at(4, 2, 1);
        BlockPos funnel2 = util.grid().at(6, 2, 1);
        Selection f3s = util.select().position(5, 2, 3);
        Selection f2s = util.select().position(funnel2);
        Selection f1s = util.select().position(funnel1);
        Selection barrel = util.select().position(5, 2, 1);
        BlockPos repack = util.grid().at(5, 2, 2);
        ...
        scene.world().showSection(crafterScaff, Direction.NORTH);
        scene.idle(3);
        scene.world().showSection(crafter, Direction.DOWN);
        scene.idle(5);
        scene.world().showSection(packager, Direction.WEST);
        ...
        scene.world().createItemOnBelt(util.grid().at(3, 1, 1), Direction.DOWN, box1);
        scene.world().createItemOnBelt(util.grid().at(2, 1, 1), Direction.DOWN, box2);
        scene.idle(13);
        ...
        scene.world().showSection(repackS, Direction.NORTH);
        scene.rotateCameraY(-15);
        scene.idle(15);

        scene.overlay().showText(80).text("When this is the case, redirect packages to an inventory").attachKeyFrame()
            .pointAt(util.vector().blockSurface(util.grid().at(5, 2, 1), Direction.WEST)).placeNearTarget();
        scene.idle(90);

        scene.overlay().showText(60).text("Attach a re-packager, and power it with redstone").attachKeyFrame()
            .pointAt(util.vector().blockSurface(util.grid().at(5, 2, 2), Direction.WEST)).placeNearTarget();
        scene.idle(40);

        scene.world().toggleRedstonePower(repackS);
        scene.effects().indicateRedstone(util.grid().at(5, 3, 2));
        scene.idle(40);

        scene.world().restoreBlocks(f1s);
        scene.world().restoreBlocks(f2s);
        scene.world().showSection(f1s, Direction.DOWN);
        scene.world().showSection(f2s, Direction.DOWN);
        scene.idle(5);
        scene.world().showSection(f3s, Direction.DOWN);
        scene.idle(20);

        scene.world().createItemOnBelt(util.grid().at(3, 1, 1), Direction.DOWN, box1);
        scene.world().createItemOnBelt(util.grid().at(1, 1, 1), Direction.DOWN, box2);
        scene.world().createItemOnBelt(util.grid().at(7, 1, 1), Direction.DOWN, box3);
        scene.idle(23);
        scene.world().removeItemsFromBelt(util.grid().at(4, 1, 1));
        scene.world().removeItemsFromBelt(util.grid().at(6, 1, 1));
        scene.world().flapFunnel(util.grid().at(4, 2, 1), false);
        scene.idle(63);
        scene.world().removeItemsFromBelt(util.grid().at(4, 1, 1));
        scene.world().flapFunnel(util.grid().at(4, 2, 1), false);
        scene.idle(3);
