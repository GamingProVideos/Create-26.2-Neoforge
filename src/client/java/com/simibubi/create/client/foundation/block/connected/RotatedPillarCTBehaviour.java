...
public class RotatedPillarCTBehaviour extends HorizontalCTBehaviour {
    ...
    public boolean connectsTo(
        ...
        if (primaryOffset != null && primaryOffset.getAxis() != stateAxis && !ConnectedPillarBlock.getConnection(
            state,
            primaryOffset
        )) {
            return false;
    ...
    protected boolean isBeingBlocked(
        ...
        return state.getValue(LayeredBlock.AXIS) == face.getAxis() && super.isBeingBlocked(
            state,
            reader,
            pos,
            otherPos,
